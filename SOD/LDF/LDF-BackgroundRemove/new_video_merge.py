...
def video_merger(ori_video:str, ori_mask:str, bg_video:str, fps:int, dst_path:str):
    ...
    if rows_bg >= rows_original and cols_bg >= cols_original:
        ...
        case = 4
#https://github.com/farseerfc/ydcv-rsh.ceil(bg.shape[0]*ratio))
    if case == 1:
    ...
    for i in tqdm(range(ori_len)):
        ...
        cv2.imwrite(os.path.join(temp_out, "out{}.jpg".format(i+1)), result_frame)
        
        #videoWrite.write(result_frame)
    
    #Use ffmpeg to merge video
    command = "ffmpeg -f image2 -framerate {} -i {}/out%d.jpg -r {} {}".format(int(fps), temp_out, int(fps), dst_path)
    print(command)
    subprocess.call(command, shell=True)
    command = "rm {}/*.jpg".format(temp_out)
    subprocess.call(command, shell=True)
